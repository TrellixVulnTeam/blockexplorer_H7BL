...
function showTx(show_tx) {
  ...
  function pageButtons(pages) {
    ...
    wrapper.innerHTML =
      `<button value=${1} class="page page-link" style="border-radius: 4px 0 0 4px; border: 0; color: #000000">&#171;</button>` +
      wrapper.innerHTML;
    wrapper_down.innerHTML =
      `<button value=${1} class="page page-link" style="border-radius: 4px 0 0 4px; border: 0; color: #000000" onclick="window.scroll(0,0);">&#171;</button>` +
      wrapper_down.innerHTML;
  ...
  function buildTable() {
    ...
    for (let i in myList) {
      ...
      function dateChange() {
        //ФОРМАРИТУЕТ ДАТУ TODO ИСПРАВИТЬ ВРЕМЯ И ДАТУ, НЕ СООТВЕТСТВУЕТ СТОРОННИМ РЕСУРСАМ
        let b = myList[i][1].replace(/[T\Z\-]/gi, " "); //удаляет ненужный мусор из строки с датой
        let c =
          b.substr(8, 2) + b.substr(4, 4) + b.substr(0, 4) + b.substr(10, 10); //расставляет символы в правильном порядке
        return c;
      }
      ...
      if (myList[i][3] < 300) {
        //ЕСЛИ ПОДТВЕРЖДЕНИЙ МЕНЬШЕ ЧЕМ 6
        if (myList[i][2] < 0) {
          var row = `
          <div class="h-10 alert alert-warning" style="box-shadow: 0 0 5px rgba(0,0,0,0.5);">
            ...
            <span id="tx_hash" class="tx_hash"><a href="#" class="hash_link"> ${
              myList[i][0]
            }</a></span>
          </div>
           `;
